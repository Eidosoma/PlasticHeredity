#!/usr/bin/env python3
"""Exact-environment BGM worker for S19-L18 prefix and suffix audits."""
...
from e01_breakinggrn_transfer_audit.core import run_breaking_transfer  # noqa: E402
from e01_frozen_timebase_ensemble.core import frozen_clr  # noqa: E402
...
def main() -> None:
    ...
    states = np.load(args.input, allow_pickle=False)["states"].astype(np.int64, copy=False)
    metadata: dict[str, object] = {"status": "WORKER_STARTED", "reason": None}
    ...
    except Exception as exc:  # noqa: BLE001 - failure provenance is serialized.
        metadata.update(status="UNREGISTERED_WORKER_EXCEPTION", reason=f"{type(exc).__name__}:{exc}")
