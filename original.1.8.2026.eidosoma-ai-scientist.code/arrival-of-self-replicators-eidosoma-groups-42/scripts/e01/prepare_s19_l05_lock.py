#!/usr/bin/env python3
"""Materialize and validate the pushed pre-outcome lock for E01/S19-L05."""
...
def clean_pushed_lock() -> dict[str, Any]:
    ...
    result = {
        "schema": "eidosoma.e01.s19_l05_preoutcome_repository_lock.v1",
        "loopId": LOOP_ID,
        "branch": branch,
        "head": head,
        "remoteHead": remote,
...
def immutable_baseline_and_validation() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for loop in ("L01", "L02", "L03", "L04"):
        root = ARTIFACT_ROOT / f"loops/{loop}"
        rows.extend(
            manifest_rows(
                root, root / "artifact_manifest.json", f"IMMUTABLE_S19_{loop}"
            )
        )
    ...
    for row in ordered:
        ...
        digest.update(b"\n")
    baseline = {
        "schema": "eidosoma.e01.s19_l05_immutable_prior_baseline.v1",
        "loopId": LOOP_ID,
        "historicalBoundary": "S01-S18_V1_V2_S19-L01_S19-L02_S19-L03_S19-L04_AND_S17_WAIVER",
    ...
    validation = {
        "schema": "eidosoma.e01.s19_l05_immutable_prior_validation.v1",
        "fileCount": len(ordered),
        "aggregateSha256": digest.hexdigest(),
        "missing": [],
        "mismatches": [],
...
def synthetic_trajectory() -> Any:
    rng = np.random.Generator(
        np.random.PCG64DXSM(derive_seed128("synthetic-benchmark"))
    )
...
def synthetic_benchmark() -> dict[str, Any]:
    ...
    rng = np.random.Generator(
        np.random.PCG64DXSM(derive_seed128("permutation-benchmark"))
    )
    orders = np.vstack([rng.permutation(100) for _ in range(benchmark_replicates)])
    started = time.process_time()
    recomputed_generation_block_metrics(states, generations, orders)
    ...
    projected_suffix_cpu = (
        label_seconds * 200 * len(SUFFIX_ENDPOINT_QUANTILES) * 4 / 3600
    )
    ...
    return {
        "schema": "eidosoma.e01.s19_l05_compute_benchmark.v1",
...
def exact_preanalysis_replay() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    summary = {
        "schema": "eidosoma.e01.s19_l05_preanalysis_replay_validation.v1",
        ...
        "failureAction": "LOOP_FAILED_CLOSED_BEFORE_NEW_PAST_ONLY_LABEL_ANALYSIS",
    }
        ...
        rows.append(
            {
                ...
                "referenceRule": "NOT_APPLICABLE"
                if item.comparator_only
                else "S_LE_T_MINUS_2_AND_ZERO_LT_GS_LT_G",
                "visitCountRule": "NOT_APPLICABLE"
                if item.comparator_only
                else "UNIQUE_EARLIER_GENERATIONS",
                "backfill": False,
                "carryForward": False,
            }
        )
    return {
        "schema": "eidosoma.e01.s19_l05_label_registry.v1",
        "loopId": LOOP_ID,
        "outcomeAccessedAtLock": False,
        "labelCount": 2,
        "structuralCandidateCount": 1,
        "labels": rows,
        "fixedL04ReferenceLabelId": "MOL_CROSS_GENERATION_RECURRENCE_H900",
        "fixedL04ReferenceRecomputed": False,
            ...
            rows.append(
                {
                    ...
                    "derivedSeed": str(
                        derive_seed128(
                            candidate, matrix, "generation_block_permutation"
                        )
                    ),
                    "rootHex": ROOT_SEED_HEX,
                    "generator": "PCG64DXSM",
            ...
            for endpoint_ordinal in range(len(SUFFIX_ENDPOINT_QUANTILES)):
                for variant in ("SHUFFLE", "REPLACE"):
                    rows.append(
                        {
                            "loopId": LOOP_ID,
                            "streamDomain": "S19_L05_SUFFIX_AUDIT",
...
def append_candidate_registry() -> pd.DataFrame:
    ...
    ids = ["S19-L05-LABEL-01", "S19-L05-LABEL-02"]
    if existing["candidateId"].isin(ids).any():
        raise RuntimeError(
            "L05 candidate rows already exist; append-only replay refused"
        )
    positives = [
        "sourceGrounding",
        ...
        rows.append(
            {
                ...
                "bundleId": "L05_PAST_ONLY_CROSS_GENERATION_RECURRENCE_ACTIVATION",
                "selected": True,
                **positive,
                **penalty,
                "proposedSpecification": item.label_id,
                "selectionReason": "human-directed singleton past-only falsification plus frozen adjacent comparator",
                "rankingScore": score,
                "frozenRank": 0,
    ...
    ranks = {
        ordinal: rank + 1
        for rank, (ordinal, _) in enumerate(
...
def source_rows(retrieved: str) -> pd.DataFrame:
    sources = [
        (
            "L05_PAPER_INHERITED_RECURRENCE",
            "PRIMARY_PAPER",
            PAPER,
            "arXiv:2607.28250v1",
            "CC-BY-4.0",
            "Paper describes recurring compositions inherited across generations but does not specify a one-sided activation algorithm.",
        ),
        (
            "L05_PAPER_MARKDOWN_CONTEXT",
            "PRIMARY_PAPER_DERIVED_TEXT",
            PAPER_MARKDOWN,
            "docling-derived-from-arXiv:2607.28250v1",
            "DERIVED_FROM_CC-BY-4.0_INPUT",
            "Local paper text preserves recurrence, entry/exit, Figure 1C cluster, and Table 1 temporal fingerprints.",
        ),
        (
            "L05_HISTORICAL_GARD_GENERATION_TRACE",
            ...
            "Historical code supports generation-level recurrence context but not the exact L05 one-sided molecular rule.",
        ),
        (
            "L05_S13Y_FROZEN_INPUTS",
        ...
        (
            "L05_L04_FIXED_RETROSPECTIVE_EVIDENCE",
            "FROZEN_INTERNAL_EVIDENCE",
            L04_ROOT / "artifact_manifest.json",
            "E01-S19-L04-CROSS-GENERATION-RECURRENCE-MEMBERSHIP-v1.0.0",
            "INTERNAL_GENERATED_EVIDENCE",
            "L04 symmetric completed-run label is comparison evidence only and is not recomputed or reinterpreted.",
        ),
    ]
    rows = []
        ...
        rows.append(
            {
                ...
                if public
                    ...
                    "https://arxiv.org/abs/2607.28250v1"
                    if "PRIMARY_PAPER" in source_type
                    else None
                ),
                ...
                if public
                    ...
                    if "PRIMARY_PAPER" in source_type
                    else "FROZEN_INTERNAL_EVIDENCE"
                ),
                "finding": finding,
                "redistributionStatus": "HASH_AND_IDENTITY_ONLY"
                if public
                else "INTERNAL_OR_CITABLE_REFERENCE",
            }
        )
    return pd.DataFrame(rows)
...
def append_root_ledgers(timestamp: str) -> pd.DataFrame:
    ...
    if existing_sources["sourceId"].isin(additions["sourceId"]).any():
        raise RuntimeError("L05 source rows already exist")
    pd.concat([existing_sources, additions], ignore_index=True).to_parquet(
    ...
    if ledger["loopId"].eq(LOOP_ID).any():
        raise RuntimeError("L05 self-improvement rows already exist")
    row = {
        "ledgerSequence": int(ledger["ledgerSequence"].max()) + 1,
        "timestampUtc": timestamp,
        "loopId": LOOP_ID,
        "recordPhase": "PRE_LOOP_BELIEF_AND_SELECTION",
        "beliefBeforeLoop": "L04's directional improvement may depend on symmetric completed-run membership and future backfilling.",
        "motivatingEvidence": "L04 improved every locked fingerprint dimension but was retrospective and failed generation-order falsification.",
        "failureOrAmbiguityTargeted": "Whether a strictly one-sided earlier-generation recurrence rule creates a meaningful online activation without future suffix information.",
        "selectedHypotheses": "Exactly one strict-H>0.9, s<=t-2, earlier-positive-generation recurrence activation label plus adjacent-H comparator; L04 fixed evidence only.",
        "learned": None,
        "weakenedHypotheses": None,
        "remainingPlausibleHypotheses": None,
        "proposedNextTest": "Pending bounded L05 execution and mandatory human review.",
        "informationGainRationale": "This is a direct falsification of L04 future dependence, not another threshold, cluster, projection, emergence, or downstream search.",
        "appendOnly": True,
    }
    pd.concat(
    ...
    with (ARTIFACT_ROOT / "SELF_IMPROVEMENT_LEDGER.md").open(
        ...
        handle.write(
            "\n## Entry 009 — S19-L05 pre-loop belief and selection\n\n"
            "- **Belief before the loop:** L04's directional change may be confined to symmetric completed-run membership and backfilling.\n"
            "- **Motivating evidence:** L04 improved the joint fingerprint but used future observations and failed its generation-order control.\n"
            "- **Failure or ambiguity targeted:** Whether cross-generation recurrence creates an online one-sided activation.\n"
            "- **Selected hypothesis:** One strict-`H>0.9`, `s<=t-2`, earlier-positive-generation rule; adjacent H is comparator-only and L04 is fixed evidence only.\n"
            "- **Expected information gain:** Directly tests future dependence without adding a threshold or structural family.\n"
            "- **What was learned / weakened / remains plausible:** Pending locked execution.\n"
            "- **Next test:** Pending; mandatory human review follows L05.\n"
        )
    with (ARTIFACT_ROOT / "source_search_report.md").open(
        "a", encoding="utf-8"
    ) as handle:
        handle.write(
            "\n## S19-L05 additive source refresh — past-only recurrence activation\n\n"
            "The paper supports recurrence inherited across generations but does not identify a one-sided online molecular-state algorithm. Historical GARD sources support generation-level compositional recurrence without fixing this rule. L05 therefore executes only the human-directed falsification of L04's future dependence: strict historical cosine `H>0.9` to a nonadjacent state in an earlier positive generation, with no future reference, backfill, carry, persistence, threshold, cluster, boundary, alignment, or modal variant. No author was contacted, no new web search was required, and no unlicensed source was copied into artifacts.\n"
        )
    with (ARTIFACT_ROOT / "continuation_decision.md").open(
        "a", encoding="utf-8"
    ) as handle:
        handle.write(
            "\n## Additive human decision — authorize S19-L05 only\n\n"
            "The human selected Option 2 and authorized only `E01-S19-L05-PAST-ONLY-CROSS-GENERATION-RECURRENCE-ACTIVATION-v1.0.0`. L05 tests one one-sided past-only recurrence activation label on frozen S13Y trajectories and stops for mandatory human review. L06, S20, E02, author contact, and report generation remain inactive.\n"
        )

    registry_path = ARTIFACT_ROOT / "loop_registry.yaml"
    registry = yaml.safe_load(registry_path.read_text(encoding="utf-8"))
    if any(item["loopId"] == LOOP_ID for item in registry["loops"]):
        raise RuntimeError("L05 loop registry row already exists")
    ...
    history["history"].append(
        {
            "date": timestamp[:10],
            "decision": "CONTINUE_S19_OPTION_2_AUTHORIZE_L05_ONLY",
            "scope": VERSION,
...
def input_manifest() -> dict[str, Any]:
    paths = [
        ...
        L04_ROOT / "artifact_manifest.json",
        L04_ROOT / "fingerprint_results.parquet",
        L04_ROOT / "paper_fingerprint_comparison.csv",
        Path("/artifacts/research_steps/S08/preregistration.yaml"),
        PAPER,
        PAPER_MARKDOWN,
    ]
    return {
        "schema": "eidosoma.e01.s19_l05_input_manifest.v1",
        "loopId": LOOP_ID,
...
def specification_ledger() -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "loopId": LOOP_ID,
                "specificationId": "L05-SPEC-001-COMPARATOR",
                "labelId": COMPARATOR_LABEL_ID,
                "role": "FROZEN_PRIMARY_COMPARATOR_ONLY",
                "threshold": 0.9,
                "referenceRule": "ADJACENT_INCOMING_NOT_CROSS_GENERATION",
            ...
            {
                "loopId": LOOP_ID,
                "specificationId": "L05-SPEC-002-PRIMARY",
                "labelId": STRUCTURAL_LABEL_ID,
                "role": "PRIMARY_STRUCTURAL_EXPLORATORY",
                "threshold": 0.9,
                "referenceRule": "S_LE_T_MINUS_2_AND_ZERO_LT_GS_LT_G_UNIQUE_GENERATIONS",
                "futureReferences": False,
                "backfill": False,
                "carryForward": False,
                "selected": True,
...
def main() -> None:
    ...
    if LOOP_ROOT.exists():
        raise RuntimeError("L05 artifact directory already exists; overwrite refused")
    ...
    if not gates:
        ...
        raise RuntimeError("L05 failed a mandatory preanalysis gate")

    candidates = append_candidate_registry()
    candidates.to_csv(LOOP_ROOT / "candidate_ranking.csv", index=False)
    bundle = {
        "schema": "eidosoma.e01.s19_l05_candidate_bundle_registry.v1",
        "loopId": LOOP_ID,
        "bundleCount": 1,
        "bundles": [
            {
    ...
    write_json(
        ...
        {
            "schema": "eidosoma.e01.s19_l05_source_snapshot_manifest.v1",
            "loopId": LOOP_ID,
    ...
    write_json(
        ...
        {
            "schema": "eidosoma.e01.s19_l05_preparation_runtime.v1",
