...
session_name="grnf12_${$}_${RANDOM}"
log_file="${launcher_dir}/campaign.log"
command_text="exec env PYTHONPATH='${project_dir}/src' JAX_PLATFORMS=cuda XLA_PYTHON_CLIENT_PREALLOCATE=false OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 '${python_bin}' -m grn_f12_realistic campaign --run '${run_dir}' --profile '${profile}' >'${log_file}' 2>&1"
