...
if [[ "${mode}" != "--child" ]]; then
  ...
  echo "Detached dynamic-regime campaign launched with PID ${child_pid}"
...
printf '%s\n' "$$" >"${pid_file}"
cd "${codex_root}"
remaining="$(${python_bin} -m reviewer_lineage_identity_response.followup_dynamic_regime_cleanroom.run_campaign remaining-hard)"
...
printf '[%s] starting dynamic-regime campaign with %s workers; cumulative hard remainder %ss\n' "$(date -u +%FT%TZ)" "${workers}" "${remaining}"
"${python_bin}" -m reviewer_lineage_identity_response.followup_dynamic_regime_cleanroom.run_campaign all --workers "${workers}"
printf '[%s] dynamic-regime campaign process exited\n' "$(date -u +%FT%TZ)"
