...
if [[ "${mode}" != "--child" ]]; then
  ...
  echo "Detached plasticity-persistence campaign launched with PID ${child_pid}"
...
printf '%s\n' "$$" >"${pid_file}"
cd "${codex_root}"
remaining="$(${python_bin} -m reviewer_lineage_identity_response.followup_plasticity_persistence_cleanroom.run_campaign remaining-hard)"
...
(
  sleep "${remaining}"
  printf '[%s] hard watchdog terminating process group %s\n' "$(date -u +%FT%TZ)" "${campaign_group}" >>"${log_file}"
...
printf '[%s] starting plasticity-persistence campaign with %s workers; hard remainder %ss\n' "$(date -u +%FT%TZ)" "${workers}" "${remaining}"
"${python_bin}" -m reviewer_lineage_identity_response.followup_plasticity_persistence_cleanroom.run_campaign all --workers "${workers}"
printf '[%s] campaign process exited\n' "$(date -u +%FT%TZ)"
