...
> **Document status (15 August 2026): Historical E01-only draft.** This document predates the independent Codex and Fable adversarial programs and is not the current cross-branch paper synthesis. Its original “regime switching,” mechanistic and no-intervention language was later narrowed or superseded. Use [`replication/REPORT.md`](replication/REPORT.md), [`replication/INTERVENTIONS.md`](replication/INTERVENTIONS.md), [`replication/PHASE_G.md`](replication/PHASE_G.md), [`replication/STRICT8_REPLICATION_PREREGISTRATION.md`](replication/STRICT8_REPLICATION_PREREGISTRATION.md), and the [root discovery log](../DISCOVERY_LOG_EIDOSOMA_AI_SCIENTIST.md) for the current evidence boundary.
...
This manuscript reports both the constraints and the resulting positive finding. The constraints matter: public evidence did not support the target paper's prospective or causal conclusions under the tested reconstruction. The positive result is different: a completely frozen past-observable coordinate predicts the independently measured probability that compositional inheritance will break and a new short hereditary episode will form. We refer to this bounded simulator phenomenon as **plastic-heredity break-and-renewal**; later coherence tests showed that the three-fission endpoint does not establish a distinct new regime.
...
At this draft's E01 cutoff, the proposed E02 would treat the L54 coordinate as a **candidate replacement causal-architecture variable**, not as PhiID, paper replication, or causal control. The planned program required locks for future-suffix leakage, calibration, candidate consistency, numerical robustness, preprocessing and target sensitivity, and incremental value beyond exact H, direct history, ordinary stability, phase, and matrix propensity. PhiID remained a nonprivileged comparator, and intervention work had not yet been authorized. The later independent branches, documented outside this historical draft, supplied many of those tests.
...
Continuous evidence agreed: mean H gain toward the old anchor at resumption was about −0.26 in every cohort and candidate. Count-matched order tests found positive excess for three-fission episodes (0.026–0.032) and five-fission episodes (0.043–0.060). The original L44 estimate of `0.015–0.022 bits per transition` was later withdrawn because the IID model was fitted and scored on different symbol supports; later support-matched clean-room analyses established first-order and duration-dependent prediction without establishing biological memory or error correction.
...
The process analysis suggests a different ontology. Parent–daughter similarity is common but not uninterrupted. When it breaks, a locally hereditary episode usually reappears, generally at a composition farther from the old anchor. Temporal order and dwell duration exceed an IID inheritance-frequency baseline in later support-matched analyses. Thus the system exhibits a persistent **capacity for heredity** across changing molecular realizations.

“Plastic heredity” is deliberately narrower than homeostasis or regime switching. Exact restoration was nearly absent, and registered functional variables did not show a residual restored function beyond composition and chronology. The data instead support stochastic breaks followed by renewed local hereditary episodes. This aligns with the broader GARD idea that compositional assemblies can carry heritable organization, while modifying the assumption that one privileged composition is the operative replicator.

### What the confirmed coordinate contains

The original algorithm comparison showed that neither its beta-only nor its direct-history comparator matched the full coordinate. Beta-only graph features carried little state-local information, while direct heredity history ranked much of the stable risk. This comparison did not by itself isolate a necessary network–state interaction; later clean-room decompositions disagreed about that mechanism. The full coordinate can be written descriptively as:

\[
\text{catalytic network}\times\text{current composition/state}\times\text{recent hereditary history and phase}.
\]

The exact mechanistic compression remains unresolved. The frozen PCA/ridge coordinate is compact enough to transfer but is not a symbolic physical law, and the original grouped comparison is an algorithmic contrast rather than a causal decomposition.

### Relation to the original paper's broader premise

The confirmed result supports a cautious version of the target paper's broad organizational premise: before a future self-maintaining event, a catalytic assembly's present organization can carry information about that event's probability. The result does not support the paper's chosen quantity or event. The signal is not PhiID, the event is not first arrival of one replicator, and no intervention had shown causal control at this E01 cutoff.
...
9. **No causal intervention at the E01 cutoff.** Changing a predictor is not the same as changing the process probability. This historical draft predates Fable's later registered molecular-swap and closed-loop control experiments; their results belong to the extended branch report, not to E01.
...
At this historical cutoff, the proposed next stage was to falsify the coordinate through E02 sensitivity and simpler-control tests, followed—only if those passed—by matched stochastic futures testing whether molecular edits changed process probability. The later Fable branch performed such interventions and must be cited from the extended report; they are not retroactive E01 evidence. Broader claims still require their own registered gates.
